...
export function CreatePostScreen({ session, onCreated }: { session: Session; onCreated: () => void }) {
  ...
  const upload = async () => {
    ...
    setLoading(true); setUploadProgress(0); setError("");
    uploadController.current = new AbortController();
    try {
      const form = new FormData();
      form.append("caption", caption);
      form.append("media", { uri: asset.uri, name: asset.fileName ?? `memory.${asset.type === "video" ? "mp4" : "jpg"}`, type: asset.mimeType ?? (asset.type === "video" ? "video/mp4" : "image/jpeg") } as unknown as Blob);
      await api.createPost(session, form, setUploadProgress, uploadController.current.signal);
      Alert.alert(ja ? "保存しました" : "Saved", ja ? "10分間はプロフィールから取り消せます。" : "You can cancel from your profile for 10 minutes.", [{ text: "OK", onPress: onCreated }]);
    } catch (e) {
      if (e instanceof Error && e.message === "UPLOAD_CANCELED") setError(ja ? "アップロードを中止しました。入力内容は残っています。" : "Upload canceled. Your input is still here.");
      else setError(e instanceof Error ? e.message : "Error");
    } finally { setLoading(false); uploadController.current = null; }
  };
  const remaining = status ? Math.max(0, status.limit - status.used) : 0;
  return <ScrollView contentContainerStyle={styles.container}><View><Title>{ja ? "今日を残す" : "Keep today"}</Title><Body muted>{status?.isAnniversary ? (ja ? `今日は記念日です。あと${remaining}件投稿できます。` : `It's your anniversary. ${remaining} slots left.`) : (ja ? `今日はあと${remaining}件投稿できます。` : `${remaining} post left today.`)}</Body></View><Card>{asset?.type === "image" && <Image source={{ uri: asset.uri }} style={styles.preview} />}{asset?.type === "video" && <View style={styles.video}><Text style={styles.videoText}>▶ {asset.fileName ?? (ja ? "選択した動画" : "Selected video")}</Text></View>}{asset && !loading && <Button secondary label={ja ? "選択を解除" : "Remove selection"} onPress={() => setAsset(null)} />}<Button secondary label={ja ? "カメラで写真を撮る" : "Take a photo"} onPress={takePhoto} disabled={remaining === 0 || loading} /><Button secondary label={session.user.membership === "premium" ? (ja ? "写真・動画を選ぶ" : "Choose photo or video") : (ja ? "写真を選ぶ" : "Choose a photo")} onPress={choose} disabled={remaining === 0 || loading} /><Field editable={!loading} placeholder={ja ? "一年後の自分へのメッセージ（任意）" : "A message to your future self (optional)"} value={caption} onChangeText={setCaption} multiline maxLength={2000} style={styles.message} /><Body muted>{caption.length}/2000</Body>{loading && <><View style={styles.progressTrack}><View style={[styles.progressValue, { width: `${uploadProgress}%` }]} /></View><Body>{ja ? `アップロード中 ${uploadProgress}%` : `Uploading ${uploadProgress}%`}</Body><Button danger label={ja ? "アップロードを中止" : "Cancel upload"} onPress={() => uploadController.current?.abort()} /></>}{!!error && <Text style={styles.error}>{error}</Text>}<Button label={ja ? "一年後へ保存" : "Save for next year"} onPress={submit} loading={loading} disabled={!asset || remaining === 0} /></Card><Body muted>{ja ? "位置情報は取得・保存しません。" : "Location information is not collected or stored."}</Body></ScrollView>;
}
const styles = StyleSheet.create({ container: { padding: spacing.lg, gap: spacing.lg }, preview: { width: "100%", aspectRatio: 1, borderRadius: 14 }, video: { minHeight: 120, borderRadius: 14, backgroundColor: colors.primarySoft, alignItems: "center", justifyContent: "center" }, videoText: { color: colors.primary, fontWeight: "700" }, message: { minHeight: 110, textAlignVertical: "top" }, progressTrack: { height: 10, borderRadius: 5, backgroundColor: colors.border, overflow: "hidden" }, progressValue: { height: "100%", backgroundColor: colors.primary }, error: { color: colors.danger } });
