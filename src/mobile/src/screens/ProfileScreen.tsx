...
type ProfilePage = "home" | "edit" | "subscription" | "legal" | "account";
...
export function ProfileScreen({ session, onSessionChange }: { session: Session; onSessionChange: (session: Session | null) => void }) {
  ...
  return <ScrollView contentContainerStyle={styles.container}>
    ...
    {!!todayStatus?.cancelablePosts.length && <Card><Text style={styles.section}>{ja ? "取り消し可能な投稿" : "Cancelable posts"}</Text><Body muted>{ja ? "投稿から10分以内です。" : "Posted within the last 10 minutes."}</Body>{todayStatus.cancelablePosts.map((post, index) => <View key={post.id} style={styles.cancelRow}><Body>{new Intl.DateTimeFormat(ja ? "ja-JP" : "en-US", { hour: "2-digit", minute: "2-digit", timeZone: session.user.timezone }).format(new Date(post.createdAt))}</Body><Button danger label={ja ? `${index + 1}件目を取り消す` : `Cancel post ${index + 1}`} onPress={() => cancelPost(post.id)} /></View>)}</Card>}
    ...
    <Card><Text style={styles.section}>{ja ? "メニュー" : "Menu"}</Text><Button secondary label={ja ? "Premiumプラン" : "Premium plan"} onPress={() => setPage("subscription")} /><Button secondary label={ja ? "アカウント管理" : "Account settings"} onPress={() => setPage("account")} /><Button secondary label={ja ? "利用規約・プライバシー" : "Terms & privacy"} onPress={() => setPage("legal")} /><Body muted>{ja ? "位置情報は使用しません。" : "Location is not used."}</Body></Card>
...
const styles = StyleSheet.create({
  ...
  section: { color: colors.text, fontSize: 18, fontWeight: "700" }, anniversary: { color: colors.text, fontSize: 18 }, row: { flexDirection: "row", gap: spacing.sm }, half: { flex: 1 }, cancelRow: { gap: spacing.sm }, error: { color: colors.danger },
});
