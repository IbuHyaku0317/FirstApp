...
| status | varchar(30) | NO | `active`, `grace_period`, `expired`, `revoked` |
...
- `uq_subscriptions_user (user_id)` UNIQUE
...
写真の撮影場所、端末の現在地、緯度、経度、住所、地名は初期リリースでは保持しない。将来追加する場合は既存投稿との互換性を保つNULL許可の別テーブルまたは列としてMigrationを行い、明示的に同意した投稿だけを対象とする。
