...
| 用途 | 技術 | 方針 |
|---|---|---|
| 動画再生 | Expo Video（SDK 57互換版） | 認証ヘッダー、ネイティブ操作、全画面再生 |
| アップロード進捗 | React Native XMLHttpRequest | 進捗通知、中止、401時のトークン更新と一度だけ再試行 |
| APIホスト統合テスト | Microsoft.AspNetCore.Mvc.Testing | 実際の認証・HTTPパイプラインを検証 |
